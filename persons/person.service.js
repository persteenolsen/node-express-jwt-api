...
class Service {
   ...
   doGetAllPersons(response, con) {
        ...
        if (err) 
            ...
            con.query("SELECT * FROM node_crud_spa_jwt ORDER BY id DESC", function (err, result, fields) {
        if (err) 
            throw err;
        else{
   ...
   doGetPerson(request, response, con, id) {
       ...
       if(isnumber == true ){
           ...
           if (err) throw err;
               ...
               con.query("SELECT * FROM node_crud_spa_jwt WHERE Id=" + id, function (err, result, fields) {
           if (err) throw err;
           else{
            ...
            if( inputdatavalid == true && isnumber == true ){
               ...
               con.connect(function(err) {
                  if (err) throw err;
                      ...
                      con.query("UPDATE node_crud_spa_jwt SET name='" + postBody.name + "', email='" + postBody.email + "', age=" + postBody.age + " WHERE Id=" + id, function (err, results, fields) {
                   if(err) throw err;
                   else{
                       response.writeHead(200, {"Content-Type": "application/json"});
           ...
           if( inputdatavalid == true ){
              ...
              con.connect(function(err) {
                 if (err) throw err;
                     ...
                     con.query("INSERT INTO node_crud_spa_jwt (name,email,age) values('" + postBody.name + "','" + postBody.email + "'," + postBody.age + ")", function (err, results, fields) {
                     if(err) throw err;
                  else{
                      response.writeHead(200, {"Content-Type": "application/json"});
                      response.end(JSON.stringify(results));
   ...
   doDeletePerson(response, con, id) {
       ...
       if (err) throw err;
           ...
           con.query("DELETE FROM node_crud_spa_jwt WHERE Id=" + id, function (err, result, fields) {
       if (err) throw err;
       else{
            response.writeHead(200, {"Content-Type": "application/json"});
            response.end(JSON.stringify(result));
